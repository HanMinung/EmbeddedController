...
// 2. Configure GPIO AFR by Pin num.
//   if(pin>=0 && pin<=7) {
//      if(TIMx==TIM1 || TIMx==TIM2)                           { port->AFR[0] &= ~(15U << (4*pin));port->AFR[0] |= 1U << (4*pin);} 
//      if(TIMx==TIM3 || TIMx==TIM4  || TIMx==TIM5)     { port->AFR[0] &= ~(15U << (4*pin));port->AFR[0] |= 2U << (4*pin);} 
//      if(TIMx==TIM9 || TIMx==TIM10 || TIMx==TIM11)   { port->AFR[0] &= ~(15U << (4*pin));port->AFR[0] |= 3U << (4*pin);} 
//   }
//   else if(pin>=8 && pin<=15) {
//      if(TIMx==TIM1 || TIMx==TIM2)                            { port->AFR[1] &= ~(15U << (4*(pin%8))); port->AFR[1] |= 1U << (4*(pin%8));} 
//      if(TIMx==TIM3 || TIMx==TIM4  || TIMx==TIM5)     { port->AFR[1] &= ~(15U << (4*(pin%8))); port->AFR[1] |= 2U << (4*(pin%8));} 
//      if(TIMx==TIM9 || TIMx==TIM10 || TIMx==TIM11)   { port->AFR[1] &= ~(15U << (4*(pin%8))); port->AFR[1] |= 3U << (4*(pin%8));} 
//   }
...
//               TIMx->CCMR1                      &= ~TIM_CCMR1_CC1S;               //reset   CC1S
//               if (ICn==CHn) TIMx->CCMR1 |=    TIM_CCMR1_CC1S_0;        //01<<0   CC1S    Tx_Ch1=IC1
//               else TIMx->CCMR1                |=    TIM_CCMR1_CC2S_1;         //10<<0   CC1S    Tx_Ch2=IC1
//               break;
//         case 2:
//               TIMx->CCMR1                      &= ~TIM_CCMR1_CC2S;               //reset   CC2S
//               if (ICn==CHn) TIMx->CCMR1 |=    TIM_CCMR1_CC2S_0;        //01<<0   CC2S    Tx_Ch2=IC2
...
//   switch(edge_type){
//      case IC_RISE: TIMx->CCER &= ~(5<<(((ICn-1)*4)+1));       break; //rising:  00
//      case IC_FALL: TIMx->CCER |= (1<<(((ICn-1)*4)+1));    break; //falling: 01
//      case IC_BOTH: TIMx->CCER |= (5<<(((ICn-1)*4)+1));    break; //both:    11
//   }
//   
// Enable CC. Enable CC Interrupt. 
//   TIMx->CCER |= 1 << (4*(ICn - 1));                               // Capture Enable
